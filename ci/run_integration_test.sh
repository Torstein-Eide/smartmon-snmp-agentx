...
cleanup() {
    ...
    [ -n "$TRAPD_PID"   ] && kill "$TRAPD_PID"   2>/dev/null || true
...
# Writable copy of fixtures so we can trigger re-parses by replacing files
LIVE_FIXTURES="$AGENTX_RUN_DIR/fixtures"
cp -r "$FIXTURES" "$LIVE_FIXTURES"
# Use the live copy as the state_dir watched by agentxd
FIXTURES="$LIVE_FIXTURES"

TRAP_LOG="$OUTPUT/trapd.log"
...
trap2sink 127.0.0.1:${TRAP_PORT} public
EOF

# ---------------------------------------------------------------------------
# Start snmptrapd (no auth, log all to file)
# ---------------------------------------------------------------------------
TRAPD_CONF="$AGENTX_RUN_DIR/snmptrapd.conf"
cat > "$TRAPD_CONF" <<EOF
disableAuthorization yes
EOF

if command -v snmptrapd >/dev/null 2>&1; then
    echo "--- starting snmptrapd on port ${TRAP_PORT} ---"
    snmptrapd -f -Lo -C -c "$TRAPD_CONF" \
        "udp:127.0.0.1:${TRAP_PORT}" \
        >> "$TRAP_LOG" 2>&1 &
    TRAPD_PID=$!
    sleep 0.5
    echo "  snmptrapd started (pid=$TRAPD_PID)"
else
    echo "  SKIP: snmptrapd not installed — trap delivery tests will be skipped"
    TRAPD_PID=""
fi
...
AGENTXD_LOG="$OUTPUT/agentxd.log"
# shellcheck disable=SC2086
"$AGENTXD_BIN" -f -c "$AGENTXD_CONF" ${AGENTXD_EXTRA_ARGS:-} >"$AGENTXD_LOG" 2>&1 &
AGENTXD_PID=$!
...
# ===========================================================================
# Trap delivery test — health-change notification
# Requires snmptrapd to be running and the failing fixture variant to exist.
# ===========================================================================
FAILING_FIXTURE="$FIXTURES/smartd-json.Samsung_SSD_980_PRO_1TB-S6B0NJ0N123456.nvme.failing.json"
LIVE_FIXTURE="$FIXTURES/smartd-json.Samsung_SSD_980_PRO_1TB-S6B0NJ0N123456.nvme.json"

if [ -n "$TRAPD_PID" ] && [ -f "$FAILING_FIXTURE" ]; then
    echo ""
    echo "  [Trap delivery — NVMe health-change notification]"

    # Replace the healthy fixture with the failing one; inotify triggers re-parse
    cp "$FAILING_FIXTURE" "$LIVE_FIXTURE"
    # Allow inotify + trap delivery time
    sleep 3

    # Notification OID for smartmonNvmeHealthChanged = 1.3.6.1.4.1.9999.1.1.3.2.2
    # (SMARTMON_ENT = enterprises.9999.1.1, then .3.2.2 for NVMe health changed)
    NOTIF_OID="1.3.6.1.4.1.9999.1.1.3.2.2"
    if grep -q "${NOTIF_OID}" "$TRAP_LOG" 2>/dev/null; then
        echo "  PASS: health-change trap received (OID ${NOTIF_OID})"
        PASS=$((PASS + 1))
    else
        echo "  FAIL: expected health-change trap not received" >&2
        echo "        OID: ${NOTIF_OID}" >&2
        echo "        trapd.log: $TRAP_LOG" >&2
        [ -f "$TRAP_LOG" ] && tail -20 "$TRAP_LOG" >&2 || true
        FAIL=$((FAIL + 1))
    fi
else
    if [ -z "$TRAPD_PID" ]; then
        echo "  SKIP: trap delivery test (snmptrapd not available)"
    else
        echo "  SKIP: trap delivery test (failing fixture not found: $FAILING_FIXTURE)"
    fi
    SKIP=$((SKIP + 1))
fi
...
# ---------------------------------------------------------------------------
{
    ...
    echo "--- trapd.log ---"
    cat "$TRAP_LOG" 2>/dev/null || echo "(missing or snmptrapd not run)"
    echo ""
